...
`result/Baseline3/hybrid-model/` (script: `src/wp0_plot_disjoint_retrain.py`, SEED=100,
...
(`Table2_model_performance.csv`, `n_total` column).
...
- WP-D: ship `src/wp0_plot_disjoint_retrain.py` + the reproducibility notes (seeded `curve_fit` `p0`,
  `geo-ml3` for CLI scipy) and `Table2_model_performance.csv` in the public repo.
