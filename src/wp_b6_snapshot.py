...
Writes `SNAPSHOT.md` into the tagged result dir. Read-only on all artifacts.
...
TAG_DIR = os.path.join(REPO, "result", "Baseline3", "hybrid-model")
FIG_DIR = os.path.join(REPO, "fig")
RASTER_DIR = r"F:\CBH"

SMALL_ARTIFACTS = [
    os.path.join(TAG_DIR, "pred_vs_NIFS_by_category.csv"),
    os.path.join(TAG_DIR, "Table2_model_performance.csv"),
    os.path.join(TAG_DIR, "baseline_eval_plot.csv"),
    os.path.join(TAG_DIR, "hybrid_eval_plot.csv"),
    os.path.join(TAG_DIR, "model", "hybrid_model.joblib"),
    os.path.join(TAG_DIR, "validation_report.txt"),
...
BIG_RASTERS = [
    os.path.join(RASTER_DIR, "CR_hybrid_model.tif"),
    os.path.join(RASTER_DIR, "CBH_meter_hybrid_model.tif"),
]
...
def main():
    ...
    out = os.path.join(TAG_DIR, "SNAPSHOT.md")

    L = []
    L.append(f"# WP-B immutable snapshot — {stamp}")
    L.append(f"\nBranch `{branch}` @ commit `{commit}` · tag `hybrid-model` · SEED 100")
    L.append("\nValidation gate: see `validation_report.txt` (12/12 PASS at snapshot time).")
